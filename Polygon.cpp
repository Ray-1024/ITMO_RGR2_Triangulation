...
void Polygon::update() {
    ...
    //std::set<std::pair<float, int> > verts;
    ...
    /*while (verts.size() > 3) {
        int v = verts.begin()->second, pr = prev[v], nx = next[v];
        verts.extract(verts.begin());
        verts.extract(verts.find({ang[pr], pr}));
        verts.extract(verts.find({ang[nx], nx}));
        addLine(pr, nx);
        next[pr] = nx;
        prev[nx] = pr;
        ang[pr] = getAngle(getVert(prev[pr]), getVert(pr), getVert(next[pr]));
        ang[nx] = getAngle(getVert(prev[nx]), getVert(nx), getVert(next[nx]));
        verts.insert({ang[pr], pr});
        verts.insert({ang[nx], nx});
    }*/
    ...
    for (int curr = 0; curr < arr.getVertexCount() - 1; ++curr) {
        sf::Vector2f a = getVert(prev[curr]), b = getVert(curr), c = getVert(next[curr]);
